# -*- coding: utf-8 -*-
"""
Created on Thu Apr 11 02:30:28 2019
...
def train_validate_test_split(df, train_percent=.7, validate_percent=.1):
    ...
    dataset_train = df[:split]
    #dataset_val = df[split_1:split_2]
    dataset_test = df[split:]
...
trainLabel2 = np.zeros((trainLabel.shape[0],5))
testLabel2 = np.zeros((testLabel.shape[0],5))
...
model = mlp(X_pca_data_train.shape[1],500,300)
...
loss_test = criterion(output_test, y_test_onehot)
ts_corr = np.sum(np.equal(preds_.cpu().numpy(),testLabel))
ts_acc = ts_corr/(testLabel.shape[0]/100)
...
ts_acc_train = ts_corr_train/(trainLabel.shape[0]/100)
print('Training accuracy = '+str(ts_acc_train))
print("\n")
print(loss_test)
print("\n")

print('Training confusion matrix:',confusion_matrix(trainLabel, preds_train.cpu().numpy()))
print('Training confusion matrix:',confusion_matrix(testLabel, preds_.cpu().numpy()))
