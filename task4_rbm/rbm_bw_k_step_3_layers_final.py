...
Z2_train = torch.from_numpy(train_features_2)
v2_train = torch.from_numpy(train_labels_2)

Z2_test = torch.from_numpy(test_features_2)
v2_test = torch.from_numpy(test_labels_2)

Z2_val = torch.from_numpy(val_features_2)
v2_val = torch.from_numpy(val_labels_2)

########## TRAINING RBM ##########
print('Training RBM...')

rbm3 = RBM(VISIBLE_UNITS, HIDDEN_UNITS, CD_K)
prev_error = 0
for epoch in range(EPOCHS):
    epoch_error = 0.0
    ...
    for i in range(int(Z2_train.size()[0]/B_Size)):
        ...
        inputs = torch.index_select(Z2_train,0,torch.linspace(i*B_Size,(i+1)*B_Size - 1,steps=B_Size)
                                  .long()).float()
        if i == list(range(int(Z2_train.size()[0]/B_Size)))[-1]:
            inputs = Z2_train[(i*B_Size):].float()
        batch_error = rbm3.contrastive_divergence(inputs)

        epoch_error += batch_error

    epoch_error = epoch_error/(Z2_train.size()[0])
    print('Epoch Error (epoch=%d): %.4f' % (epoch, epoch_error))
...
train_features_3 = np.zeros((Z2_train.size()[0], HIDDEN_UNITS))
train_labels_3 = np.zeros(Z2_train.size()[0])
test_features_3 = np.zeros((Z2_test.size()[0], HIDDEN_UNITS))
test_labels_3 = np.zeros(Z2_test.size()[0])
val_features_3 = np.zeros((Z2_val.size()[0], HIDDEN_UNITS))
val_labels_3 = np.zeros(Z2_val.size()[0])

train_features_3 = rbm3.sample_hidden(Z2_train).cpu().numpy()
train_labels_3 = v2_train.numpy() 

test_features_3 = rbm3.sample_hidden(Z2_test).cpu().numpy()
test_labels_3 = v2_test.numpy() 

val_features_3 = rbm3.sample_hidden(Z2_val).cpu().numpy()
val_labels_3 = v2_val.numpy()   
...
class mlp(nn.Module):
    def __init__(self,i,h1,h2,h3): 
        super(mlp, self).__init__()
        self.N1 = nn.Linear(i,h1)        
        self.N2 = nn.Linear(h1,h2) 
        self.N3 = nn.Linear(h2,h3)
        self.N4 = nn.Linear(h3,5)
    def forward(self,x): 
        x = self.N1(x)  
        x = F.tanh(x)
        x = self.N2(x)          
...
for i in range(trainLabel2.shape[0]):
    trainLabel2[i,train_labels_3.astype(int)[i]] = 1
for j in range(testLabel2.shape[0]):
    testLabel2[j,test_labels_3.astype(int)[j]] = 1
for k in range(valLabel2.shape[0]):
    valLabel2[k,val_labels_3.astype(int)[k]] = 1
...
model.N3.weight.data.copy_(rbm3.weights.data.t())
model.N3.bias.data.copy_(rbm3.hidden_bias.data)
