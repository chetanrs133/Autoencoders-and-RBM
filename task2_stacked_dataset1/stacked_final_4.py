# -*- coding: utf-8 -*-
"""
Created on Tue Apr  9 18:39:11 2019
...
#aann4
net4 = autoencoder(120,80,20)
print(net4)
# 
net4 = net4.float()

cr = nn.MSELoss()
opt = optim.SGD(net4.parameters(), lr=0.5, momentum=0.9)
#training
iteration = 150
B_Size = 90
for epoch in range(iteration):
    runningLoss = 0
    for i in range(int(Z3_train.size()[0]/B_Size)):
        inputs = torch.index_select(Z3_train,0,torch.linspace(i*B_Size,(i+1)*B_Size - 1,steps=B_Size)
                                  .long()).float()
        ...
        runningLoss += loss.item()
    print('At Iteration : %d / %d  ;  Mean-Squared Error : %f'%(epoch + 1,iteration,runningLoss/
                                                                (Z3_train.size()[0]/B_Size)))
print('Finished Training')

new_classifier4 = nn.Sequential(*list(net4.children())[0])
Z3_train_var = Variable(Z3_train)
Z4_train = new_classifier4(Z3_train_var)
...
e = list(new_classifier4.children())
d = a + b + c + e
...
loss_val = cr(outputs_val,labels_val).item()
_____, predicted_val = torch.max(outputs_val.data, 1)
predicted_val = predicted_val.cpu()
total_val = Y_val.size(0)
correct_val = (predicted_val == Y_val).sum()
acc_val = 100 * correct_val/ float(total_val)
